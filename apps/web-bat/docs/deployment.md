...
- `/api/v1/health/ready` probes a small `rassy-smart` chat completion by default so lane or alias drift in RassyCodex is visible before a worker cycle gets stuck. Set `LLM_READINESS_INFERENCE_PROBE_ENABLED=false` only for environments where readiness must be catalog-only.
