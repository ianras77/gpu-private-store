...
const buildCusps = (start: number, step: number): number[] => {
  const cusps: number[] = [];
...
export const computeWholeSignHouses = (ascendant: number): HouseInfo => {
...
const quadrantCusps = (
  ...
  return [c1, c2, c3, c4, c5, c6, c7, c8, c9, c10, c11, c12].map(
    normalizeDegree
  );
};

export const computePlacidusHouses = (
  ascendant: number,
  midheaven: number
): HouseInfo => {
  return {
...
const isBetween = (deg: number, start: number, end: number): boolean => {
...
export const houseForDegree = (deg: number, cusps: number[]): number => {
