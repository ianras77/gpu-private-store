...
generates embeddings via an OpenAI-compatible API, and inserts into PostgreSQL.
...
DEFAULT_EMBEDDING_API_BASE = "https://openrouter.ai/api/v1"
...
def get_embedding_api_base() -> str:
    return (os.environ.get("EMBEDDING_API_BASE") or DEFAULT_EMBEDDING_API_BASE).rstrip("/")


def get_embedding_api_key() -> str:
    return os.environ.get("EMBEDDING_API_KEY") or os.environ.get("OPENROUTER_API_KEY", "")
...
def get_chat_api_base() -> str:
    return (os.environ.get("CHAT_API_BASE") or get_embedding_api_base()).rstrip("/")


def get_chat_api_key() -> str:
    return os.environ.get("CHAT_API_KEY") or get_embedding_api_key()


def get_chat_model() -> str:
    return os.environ.get("CHAT_MODEL") or DEFAULT_CHAT_MODEL
...
def db_conninfo() -> str:
    return " ".join(
        [
            ...
            f"dbname={os.environ.get('DB_NAME', 'openbrain')}",
            f"user={os.environ.get('DB_USER', 'openbrain')}",
...
def state_dir() -> Path:
    directory = Path(os.environ.get("STATE_DIR", Path(__file__).parent)).expanduser()
...
def chunk_note(note: dict, use_llm: bool, chat_api_key: str,
    ...
    for chunk in chunks:
        if word_count(chunk['content']) > LLM_CHUNK_THRESHOLD and use_llm:
            ...
            llm_thoughts = llm_distill(title, chunk['content'], chat_api_key)
...
def llm_distill(title: str, content: str, api_key: str) -> list[str]:
    ...
    for attempt in range(MAX_RETRIES):
        try:
            resp = requests.post(
                f"{get_chat_api_base()}/chat/completions",
...
def generate_embedding(text: str, api_key: str) -> list[float] | None:
    """Generate an embedding via the configured OpenAI-compatible endpoint."""
    for attempt in range(MAX_RETRIES):
        try:
            resp = requests.post(
                f"{get_embedding_api_base()}/embeddings",
                headers=auth_headers(api_key),
                json={
            ...
            if attempt < MAX_RETRIES - 1:
                ...
                if status == 429:
                    ...
                    print(f"  Rate limited by embedding API. Retrying in {retry_after}s "
...
def insert_thought(content: str, embedding: list[float] | None, metadata: dict,
    ...
    """Insert a thought into the PostgreSQL thoughts table.
    ...
    for attempt in range(MAX_RETRIES):
        try:
            with psycopg.connect(conninfo) as conn:
                with conn.cursor() as cur:
                    cur.execute(
                        ...
                        WHERE content_fingerprint IS NOT NULL DO NOTHING
                        RETURNING id
                        """,
                        (
                            content,
            ...
            if attempt < MAX_RETRIES - 1 and sqlstate in ("40001", "40P01", "53300"):
                wait = RETRY_BACKOFF * (2 ** attempt)
                print(f"  Database retry in {wait}s (attempt {attempt + 1}/{MAX_RETRIES})", flush=True)
                time.sleep(wait)
                continue
...
def load_sync_log(directory: Path) -> dict:
    log_path = directory / SYNC_LOG_FILE
    if log_path.exists():
        try:
...
def save_sync_log(directory: Path, log: dict):
    log_path = directory / SYNC_LOG_FILE
...
def main():
    ...
    connection_info = db_conninfo()

    if not args.dry_run:
        if not db_password:
            print("Error: DB_PASSWORD is required", file=sys.stderr)
            print("Set them in .env or as environment variables", file=sys.stderr)
            sys.exit(1)
        if not embedding_api_key and not args.no_embed:
            print("Warning: EMBEDDING_API_KEY is empty; continuing in case your API does not require auth.",
                  file=sys.stderr)
    ...
    if not args.dry_run:
        ...
        try:
            with psycopg.connect(connection_info) as conn:
                with conn.cursor() as cur:
                    cur.execute("SELECT 1 FROM thoughts LIMIT 1")
                    cur.fetchone()
        except psycopg.Error as e:
            print(f"Error: could not reach PostgreSQL: {e}", file=sys.stderr)
            print("  Check DB_HOST, DB_PORT, DB_NAME, DB_USER, and DB_PASSWORD in .env", file=sys.stderr)
            sys.exit(1)

        if not args.no_embed:
            test_embedding = generate_embedding("preflight check", embedding_api_key)
            if not test_embedding:
                print("Error: embedding preflight failed.", file=sys.stderr)
                print("  Check EMBEDDING_API_BASE / EMBEDDING_API_KEY in .env and that your model endpoint is reachable.",
                      file=sys.stderr)
                sys.exit(1)

        print("  PostgreSQL and embedding connections verified.", flush=True)
        print()
    ...
    for i, note in enumerate(filtered):
        chunks = chunk_note(note, use_llm, chat_api_key, verbose=args.verbose)
    ...
    for i, thought in enumerate(all_thoughts):
        ...
        embedding = None
        if not args.no_embed:
            embedding = generate_embedding(thought['content'], embedding_api_key)
            if not embedding:
                embed_failures += 1
            else:
...
def _write_report(thoughts, notes, vault_root, args, skip_reasons,
    ...
    report_path = state_dir() / "import-report.md"
    lines = [
