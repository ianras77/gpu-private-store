...
import { z } from "zod";
import { executeWebSearch, type SearchFailureReason, type WebSearchInput, type WebSearchResult } from "@/lib/web-search";
...
export type ParallelResearchResult = { searches: Array<{ query: string; status: "ok" | "empty" | "failed"; reason?: SearchFailureReason; results: WebSearchResult[] }> };
...
export const parallelResearchTool = createTool({
  ...
  inputSchema,
  outputSchema: z.object({ searches: z.array(z.object({ query: z.string(), status: z.enum(["ok", "empty", "failed"]), reason: z.enum(["forbidden", "rate_limited", "timeout", "invalid_response", "unavailable"]).optional(), results: z.array(z.object({ title: z.string(), url: z.string(), source: z.string().optional(), publishedAt: z.string().optional(), snippet: z.string(), status: z.literal("ok").optional() })) })) }),
