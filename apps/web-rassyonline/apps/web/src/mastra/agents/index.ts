...
export const researcher = new Agent({ id: "researcher", name: "researcher", description: "Evidence-focused current research coordinator", instructions: `For every research request, call webSearch before answering. For comparisons or questions with multiple dimensions, make separate searches when useful, compare independent sources, and resolve disagreements explicitly. Never claim a search occurred without its tool result. Cite only URLs returned by webSearch, distinguish evidence from inference, and say plainly when any search failed or returned no usable results. ${safe}`, model: chatModel("rassy-agent"), memory, tools: { webSearch: rassyTools.webSearch } });
