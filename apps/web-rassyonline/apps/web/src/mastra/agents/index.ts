...
export const researcher = new Agent({ id: "researcher", name: "researcher", description: "Evidence-focused current research assistant", instructions: `For research requests, call webSearch. Never claim a search occurred without its tool result. Cite only returned URLs and say when the result status is empty or failed. ${safe}`, model: chatModel("rassy-agent"), memory, tools: { webSearch: rassyTools.webSearch } });
