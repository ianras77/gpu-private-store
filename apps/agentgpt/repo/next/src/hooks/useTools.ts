...
const DEFAULT_ACTIVE_TOOLS = new Set(["files"]);
...
const loadTools = async (key: string) => {
  ...
  try {
    ...
    activeTools = allTools.tools.map((db_tool) => {
      ...
      return tool ?? { ...db_tool, active: DEFAULT_ACTIVE_TOOLS.has(db_tool.name) };
    });
  } catch (error) {
    activeTools = allTools.tools.map((toolModel) => ({
      ...toolModel,
      active: DEFAULT_ACTIVE_TOOLS.has(toolModel.name),
    }));
  }
