...
function createTempWebDist() {
  ...
  tempDirs.push(dir);
  fs.writeFileSync(path.join(dir, 'index.html'), '<!doctype html><html><body>portal</body></html>');
  return dir;
}
...
describe('cat routes', () => {
  it('proxies Cheshire Cat chat messages and extracts the assistant reply', async () => {
    ...
    const app = await createApp(makeEnv(), { webDistRoot: createTempWebDist() });
    instances.push(app);
  ...
  it('reports LLM health through the configured upstream', async () => {
    ...
    const body = res.json() as { ok: boolean; target: string; upstreamStatus: number };
