...
export function LandingStatus() {
  ...
  return (
    <section className="panel orbit">
      ...
      {loading ? (
        <p className="muted">Checking tenant routes and user program identity...</p>
      ) : (
        <div className="checklist">
          <div className="check">Program user: {userLabel}</div>
          <div className="check">Datasets available: {datasetCount}</div>
          <div className="check">Workspace route: /studio</div>
          <div className="check">Governed flow: upload to inspect to preview to approve to run to download</div>
        </div>
      )}
