...
export async function GET(request: Request) {
  ...
  const hires = process.env.NEXT_PUBLIC_STREAM_HIRES_URL?.trim() || new URL("/api/radio/stream?quality=hires", base).toString();
  const channels = [
    ...
    { id: "hires", name: "Hi-Res Radio", nativeHighResOnly: true, available: true, outputs: [{ id: "hires", label: "Native Hi-Res Lossless", url: hires, mimeType: "audio/ogg", codec: "flac", compatibility: "native-hi-res" }] },
  ];
