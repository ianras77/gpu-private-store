...
import { parseRollInput, rollCampaignDice } from "../../../../../../lib/dm/service";
...
export async function POST(request: Request, context: Params) {
  ...
  try {
    ...
    const key = `dm-roll:${auth.session.userId}:${campaignId}`;
    ...
    const parsed = parseRollInput(body);
    const result = await rollCampaignDice(auth.session.userId, campaignId, parsed);
    return NextResponse.json(result, { headers: rateHeaders });
  } catch (error) {
    console.error("dm_roll_failed", {
    ...
    if (error instanceof Error && error.message === "invalid_dice_expression") {
      return NextResponse.json({ error: "invalid_dice_expression" }, { status: 400 });
    }
    ...
    return NextResponse.json({ error: "roll_failed" }, { status: 500 });
  }
}
