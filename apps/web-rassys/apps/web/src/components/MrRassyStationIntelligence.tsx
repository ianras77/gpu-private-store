...
export function MrRassyStationIntelligence() {
  ...
  const plays = home?.status?.playCount ?? home?.status?.plays;
  ...
  return <section className="station-intelligence mt-6 space-y-4" aria-label="Mr Rassy station intelligence">
    ...
    <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
      {statCards.map(({ label, value, detail, Icon }) => <div key={label} className="rave-chip rounded-[22px] p-4"><Icon size={16} className="text-glow" /><div className="mt-3 text-2xl font-semibold text-white">{value == null ? "—" : String(value)}</div><div className="text-[10px] uppercase tracking-[0.2em] text-cloud/55">{label}</div><div className="mt-1 text-xs text-cloud/55">{detail}</div></div>)}
    </div>
    <div className="grid gap-4 lg:grid-cols-[1.2fr_0.8fr]">
      <div className="rave-panel rounded-[26px] p-5"><div className="eyebrow">Tonight’s handpicked route</div><div className="mt-4 grid gap-3 sm:grid-cols-2">{recommendations.length ? recommendations.map((track) => <Link href={track.id ? `/mr-rassy/track/${encodeURIComponent(track.id)}` : "/mr-rassy/library"} key={trackKey(track)} className="group rounded-2xl border border-white/10 bg-white/[0.035] p-4 transition hover:border-glow/50"><div className="text-sm font-semibold text-white group-hover:text-glow">{track.title}</div><div className="mt-1 text-xs text-cloud/60">{track.artist}{track.album ? ` · ${track.album}` : ""}</div><div className="mt-3 text-[10px] uppercase tracking-[0.18em] text-cloud/45">{track.id ? "Open trackbook" : "From the current set"}</div></Link>) : <div className="text-sm text-cloud/60">The booth is still assembling the next recommendation. Check back when the set has a little more shape.</div>}</div></div>
