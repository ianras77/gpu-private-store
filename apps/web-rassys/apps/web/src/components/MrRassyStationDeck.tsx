...
export function MrRassyStationDeck({ compact = false }: { compact?: boolean }) {
  ...
  return <section className={`station-deck ${compact ? "station-deck-compact" : ""}`} aria-label={compact ? "Mr Rassy homepage radio player" : "Mr Rassy live booth"}>
    <div className="station-deck-art">{displayNow?.albumArtUrl ? <Image src={displayNow.albumArtUrl} alt={displayNow.title ?? "Current artwork"} fill sizes={compact ? "180px" : "360px"} className="object-cover" unoptimized /> : <div className="station-art-placeholder"><Waves size={48} /></div>}<div className="station-art-overlay" /><div className="station-live-pill"><span className="glow-dot h-2 w-2 rounded-full" /> {buffering ? "CATCHING THE SIGNAL" : playing ? "ON AIR" : "READY TO PLAY"}</div><div className="station-art-copy"><div className="eyebrow">Now in the room</div><div className="truncate text-xl font-semibold text-white">{displayNow?.title ?? "The next record is being cued"}</div><div className="truncate text-sm text-cloud/70">{displayNow?.artist ?? "Mr Rassy"}</div></div></div>
