...
export function MrRassyStationDeck({ compact = false }: { compact?: boolean }) {
  ...
  const send = async () => {
    ...
    setSending(true);
    try {
      const response = await fetch("/api/radio/chat", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ message: text, clientId: "homepage-station-deck", requestId: `deck-${Date.now()}` }) });
      const payload = await response.json().catch(() => null);
      setReply(payload?.reply?.message ?? payload?.reply?.text ?? (response.ok ? "Mr Rassy heard you. He’s shaping the next thought." : "The line is busy. Try that again."));
      setMessage("");
    } finally { setSending(false); }
  };
  return <section className={`station-deck ${compact ? "station-deck-compact" : ""}`}>
