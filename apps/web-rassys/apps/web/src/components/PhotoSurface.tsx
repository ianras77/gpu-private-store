"use client";

import Image from "next/image";
import { useEffect, useState } from "react";
import type { PhotoItem } from "../lib/media-controller";

const isHeifImage = (item: PhotoItem) =>
  item.kind === "image" &&
  (/\.(heic|heif)$/i.test(item.extension) || /image\/hei[cf]/i.test(item.mimeType));

type PhotoSurfaceProps = {
  item: PhotoItem;
  alt: string;
  sizes: string;
  className?: string;
  priority?: boolean;
};

export function PhotoSurface({
  item,
  alt,
  sizes,
  className,
  priority = false,
}: PhotoSurfaceProps) {
  const [convertedSrc, setConvertedSrc] = useState<string | null>(null);
  const [conversionFailed, setConversionFailed] = useState(false);
  const needsClientConversion =
    isHeifImage(item) && item.source === "local" && Boolean(item.fileUrl);
  const resolvedSrc = convertedSrc ?? item.previewUrl ?? item.fileUrl ?? "";

  useEffect(() => {
    if (!needsClientConversion || !item.fileUrl) {
      setConvertedSrc(null);
      setConversionFailed(false);
      return;
    }

    let active = true;
    let objectUrl: string | null = null;

    setConvertedSrc(null);
    setConversionFailed(false);

    void (async () => {
      try {
        const response = await fetch(item.fileUrl!, { cache: "no-store" });
        if (!response.ok) {
          throw new Error(`photo_fetch_failed_${response.status}`);
        }

        const blob = await response.blob();
        const { default: heic2any } = await import("heic2any");
        const converted = await heic2any({
          blob,
          toType: "image/jpeg",
          quality: 0.92,
        });
        const imageBlob = Array.isArray(converted) ? converted[0] : converted;

        objectUrl = URL.createObjectURL(imageBlob as Blob);
        if (!active) {
          URL.revokeObjectURL(objectUrl);
          return;
        }

        setConvertedSrc(objectUrl);
      } catch {
        if (!active) return;
        setConversionFailed(true);
      }
    })();

    return () => {
      active = false;
      if (objectUrl) {
        URL.revokeObjectURL(objectUrl);
      }
    };
  }, [item.fileUrl, needsClientConversion]);

  if (needsClientConversion) {
    if (convertedSrc) {
      return (
        <Image
          src={convertedSrc}
          alt={alt}
          fill
          sizes={sizes}
          className={className}
          unoptimized
          priority={priority}
        />
      );
    }

    return (
      <div className="absolute inset-0 flex items-center justify-center bg-black/35 px-4 text-center text-[10px] uppercase tracking-[0.28em] text-cloud/60">
        {conversionFailed ? "Photo preview unavailable" : "Preparing photo"}
      </div>
    );
  }

  if (!resolvedSrc) {
    return (
      <div className="absolute inset-0 flex items-center justify-center bg-black/35 px-4 text-center text-[10px] uppercase tracking-[0.28em] text-cloud/60">
        Photo preview unavailable
      </div>
    );
  }

  return (
    <Image
      src={resolvedSrc}
      alt={alt}
      fill
      sizes={sizes}
      className={className}
      unoptimized
      priority={priority}
    />
  );
}
