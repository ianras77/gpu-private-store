...
final class RunSessionStore: NSObject, ObservableObject {
  ...
  private func rawPayload(bootstrap: CompanionBootstrap, elevationGain: Double) -> [String: JSONValue] {
    var payload: [String: JSONValue] = [
      "capture_mode": .string("watch-native"),
      "synced_via": .string("watchos-native"),
      "companion_device_id": .string(bootstrap.phoneDeviceId),
      ...
      "heart_rate_samples": .array(heartRateSamples)
    ]
  ...
  private func finalizeRun() async {
    ...
    do {
      ...
      guard gpsPoints.count >= 2 else {
        throw RunSessionError.notEnoughRouteData
      }
      ...
      let payload = WorkoutCreatePayload(
        ...
        raw_payload_json: rawPayload(bootstrap: bootstrap, elevationGain: elevationGain),
