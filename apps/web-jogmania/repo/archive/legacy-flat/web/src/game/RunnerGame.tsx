...
import type { RunEvent, RunSummary } from '@/lib/api';
...
export default function RunnerGame({
  ...
  useEffect(() => {
    ...
    app.ticker.add(() => {
      ...
      obstacles.forEach((obstacle) => {
        ...
        const hitX =
          obstacle.sprite.x < player.x + 22 && obstacle.sprite.x + obstacle.width > player.x;
        const hitY =
          obstacle.sprite.y < player.y + 34 && obstacle.sprite.y + obstacle.height > player.y + 8;
        if (hitX && hitY) {
          finishRun('crash');
        }
      });
  ...
  return <div ref={containerRef} className="game-canvas h-full w-full" />;
}
