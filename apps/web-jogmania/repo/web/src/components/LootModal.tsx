import type { LootItem } from '@/lib/api';
  ...
  return (
    <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/80 p-4">
      <div className="pixel-border w-full max-w-md rounded-2xl bg-[#0b0b16] p-6 text-center">
        <div className="mb-4 font-pixel text-neon-pink">Loot Cache</div>
        <div className="space-y-3 text-sm">
          {items.map((item, idx) => (
            <div key={`${item.name}-${idx}`} className="rounded-xl bg-black/50 p-3">
              <div className="text-xs uppercase text-neon-yellow">{item.rarity}</div>
